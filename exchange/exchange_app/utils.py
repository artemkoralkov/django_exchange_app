...
class CurrencyExchangeService:
    ...
    def calculate_exchange(
        ...
        rate_from = (
            ...
            else self.get_exchange_rate(cursor, currency_from_id)[0]
        )
        rate_to = (
            1
            if currency_to_id == "1"
            else self.get_exchange_rate(cursor, currency_to_id)[0]
        )
        ...
        base_currency_cash = self.get_currency_cash(
            cursor, "1"
        )  # Доступно в базовой валюте
        ...
        with transaction.atomic():
            try:
                ...
                if currency_from_id != "1" and currency_to_id != "1":
                    ...
                    exchanged_amount_base, change_in_base_base, error = (
                    ...
                    if change_in_base_base > self.get_currency_cash(cursor, "1"):
                        raise ValueError(
                            "Недостаточно базовой валюты для выдачи сдачи."
                        )
                    ...
                    exchanged_amount_target, change_in_base, error = (
                        self.calculate_exchange(
                            cursor,
                            "1",
                            currency_to_id,
